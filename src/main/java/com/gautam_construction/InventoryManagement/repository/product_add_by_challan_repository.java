...
public interface product_add_by_challan_repository extends JpaRepository<product_add_by_challan, Long>{
	...
	public final static String GET_PRODUCT_FILTER_BY_SEARCH = "select new com.gautam_construction.InventoryManagement.DTO.filter_product_search_dto(pe.prod_id,p.name,p.type,p.unit,pe.quantity,pe.challan_no,pe.date_of_entry,'By Challan') FROM product_add_by_challan pe inner join product p on pe.prod_id=p.prod_id where (:prod_id IS NULL OR :prod_id = '' OR p.prod_id=:prod_id) and (:date IS NULL OR :date = '' OR pe.date_of_entry=:date)";
	...
	public final static String GET_PRODUCT_FILTER_SEARCH_BY_PRODUCT = "select new com.gautam_construction.InventoryManagement.DTO.filter_product_search_dto_by_product(pe.prod_id,p.name,p.type,p.unit,pe.quantity,pe.challan_no,pe.date_of_entry,'By Challan') FROM product_add_by_challan pe inner join product p on pe.prod_id=p.prod_id where (:prod_id IS NULL OR :prod_id = '' OR p.prod_id=:prod_id) and (COALESCE(:s_date, '') = '' OR STR_TO_DATE(pe.date_of_entry, '%Y-%m-%d') >= STR_TO_DATE(:s_date, '%Y-%m-%d'))"
			+ "  AND (COALESCE(:e_date, '') = '' OR STR_TO_DATE(pe.date_of_entry, '%Y-%m-%d') <= STR_TO_DATE(:e_date, '%Y-%m-%d')) ORDER BY pe.date_of_entry";
